    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <section className="max-w-6xl mx-auto px-4 pt-20 pb-16 text-center">
        <div className="inline-flex items-center gap-2 bg-blue-950/40 border border-blue-800/50 text-blue-400 text-xs px-4 py-1.5 rounded-full mb-6">
          <Zap size={12} /> Live · NSE + NYSE · 2,300+ stocks · ML Predictions
        </div>
          ...
          {TECH_STACK.map(({ layer, items }) => (
            <div key={layer} className="bg-gray-900/50 border border-gray-800 rounded-2xl p-5">
              ...
              <div className="flex flex-wrap gap-2">
                {items.map(item => (
                  <span key={item} className="text-xs bg-gray-800 text-gray-300 px-2.5 py-1 rounded-lg">{item}</span>
                ))}
              </div>
            </div>
          ))}
      ...
      <footer className="border-t border-gray-800/60">
        <div className="max-w-6xl mx-auto px-4 py-8 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <p className="text-xs text-gray-700">Educational only · Not SEBI-registered · Not investment advice</p>
        </div>
      </footer>
    </div>
  )
